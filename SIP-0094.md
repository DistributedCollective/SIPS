...
`LoanClosingsWith` links the stateless library `SwapsImplSovrynSwapLib` at
`0x47Dc479cA058B4BB7C4AD9D56EE3a029236b88A8` — the deployment already live and
source-verified on the explorer, whose code this release leaves untouched. (A byte-identical
copy of the library was deployed during the release process and then set aside in favor of
linking the verified original; the two differ only in their embedded self-address and
metadata fingerprint, which anyone can confirm on-chain.) The library is not registered by
Part 1 — it is referenced only inside `LoanClosingsWith`'s link reference — and its address
is needed to reproduce that module's bytecode.
