...
The report each user prepares to qualify in the contest, must have the title of the feature tested (e.g.: “**Observations on USDT Integration**”), and a small index in the first page indicating the tests covered. Also the index page must indicate the total number of pages of the report, **_and the address_** he/she has used to test the feature.
...
1. A Multi-Signature contract of 2-3 must be deployed for the treasury of the contests events.
2. The signers of this contract will be the same jury members.
3. The treasury of the contests will be held in SOV tokens.
4. In the first day of each month the traesury of Sovryn platform will transfer to the treasury of the contersts events 440 SOV tokens and at least 30,000 satoshis to pay gas fees.
5. This project as contest series, may be disincorporated at the end of each quarter by a SIP proposal.
6. New jury members can be added, by the approval of pre-existent members, by adding the user in the Multi-Signature wallet.
7. A member can be desincorporated by a quorum, with a transaction in the Multi-Signature contract.
